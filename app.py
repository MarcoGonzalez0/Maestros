...
def region_url(comuna):
    try:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute("SELECT nombreRegion FROM Region r JOIN comuna c ON r.idRegion = c.Region_idRegion WHERE c.nombreComuna = %s", (comuna,))
        nom_region = cursor.fetchone()


        if nom_region:
            nom_region = nom_region['nombreRegion']
        else:
            nom_region = None
        ...
        dict_regiones = {
                    ...
                    'Metropolitana de Santiago': "region-metropolitana-", 
                    ...
                    'Magallanes y de la Antártica Chilena': "magallanes-antartica-"
                }
        if nom_region != None:
            return dict_regiones[nom_region]
        else:
            return nom_region
